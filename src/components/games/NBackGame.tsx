...
const NBackGame = ({ onScoreChange, onGameEnd, difficulty = 'medium' }: NBackGameProps) => {
  ...
  // Generate sequence with controlled match probability
  const generateSequence = useCallback(() => {
    ...
    for (let i = 0; i < totalTrials; i++) {
      if (i >= nLevel && Math.random() < matchProbability) {
        ...
        let letter;
        do {
          letter = LETTERS[Math.floor(Math.random() * 8)]; // Use subset for easier recognition
  ...
  useEffect(() => {
    ...
    // Check if previous trial was a miss (match but no response)
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="relative w-32 h-32 flex items-center justify-center rounded-2xl border-2 border-border bg-muted/30">
        ...
        {feedback && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`absolute inset-0 rounded-2xl flex items-center justify-center ${feedback === 'correct' ? 'bg-green-500/20' : 'bg-red-500/20'}`}
          >
            {feedback === 'correct' ? (
      ...
      {currentIndex >= 0 && currentIndex < nLevel && (
        <p className="text-xs text-muted-foreground animate-pulse">
          Memorize… responding starts after {nLevel} letters
        </p>
      )}
    </div>
  );
